...
export const HeroSection: React.FC = () => {
  ...
  const headingScale = useTransform(scrollYProgress, [0, 1], [1, 0.92]);
  const headingRotateX = useTransform(scrollYProgress, [0, 1], [0, 16]);

  const portraitY = useTransform(scrollYProgress, [0, 1], [0, 90]);
  const portraitScale = useTransform(scrollYProgress, [0, 1], [1, 1.08]);
  const portraitZ = useTransform(scrollYProgress, [0, 1], [0, 60]);

  const smoothHeadingY = useSpring(headingY, { stiffness: 120, damping: 25 });
  const smoothHeadingRotateX = useSpring(headingRotateX, { stiffness: 120, damping: 25 });
  const smoothPortraitY = useSpring(portraitY, { stiffness: 120, damping: 25 });
  const smoothPortraitScale = useSpring(portraitScale, { stiffness: 120, damping: 25 });
  ...
  return (
    ...
    >
      ...
      {/* Hero Portrait (Absolute Center) with 3D Parallax & Depth */}
      <motion.div
        style={{
          y: smoothPortraitY,
